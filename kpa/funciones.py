...
from kpa.colorprints import blue, console, green, red, yellow
...
def pkgbuild(
    ...
    verbose: bool = False,
    ignore_pkgbuild: bool = False,
    show_install_script: bool = False,
    ...
    if (
        datos["eula_detector"]
        and (not actualizacion)
        and eula_detectado(join(RUTA, paquete), pkg)
    ):
        value = confirm(
    ...
    if (not actualizacion) or show_install_script:
        try:
            ...
            try:
                ...
                yellow(
                    f"ADVERTENCIA: El script install parece usar un nombre complejo que no se pudo resolver: {install_script}"
                )
    ...
    if value:
        ...
        if len(en_aur_limpio) != 0:
            ...
            for paquete_aur in en_aur_limpio:
                ...
                try:
                    sb.run(
                        ["pacman", "-Qi", paquete_aur], check=True, capture_output=True
                    )
                except (
                    sb.CalledProcessError
                ):  # Error producido cuando el paquete no está instalado
...
def instalar(
    ...
    reinstall: Annotated[
        bool, Option(help="Reinstalar el paquete en vez de intentar instalarlo.")
    ] = False,
    show_install_script: Annotated[
        bool, Option(help="Mostrar el script install incluso en una reinstalación.")
    ] = False,
):
    for paquete in paquetes:
        ...
        try:
            ...
            if confirm(
                """ADVERTRNCIA: El repositorio ya estaba clonado, si su intención es
actualizar use el argumento Act""",
...
class Updater:
    ...
    def actualizar_uno(self, paquete: str):
        if exists(join(RUTA, paquete)):
            if paquete in datos["ignorar"] and (
                not self.ignorados and not self.solo_ignorados
            ):
                value = confirm(
            ...
            red(f"ERROR: {paquete} no ha sido clonado, para ello use el argumento Ins")
        ...
        for paquete in self.paquetes:
            if paquete == "todo":
                for directorio in listdir(RUTA):
                    if (
                        not ((not self.ignorados) and directorio in datos["ignorar"])
                        and not self.solo_ignorados
                        or self.solo_ignorados
                        and directorio in datos["ignorar"]
                    ):
...
@cli.command(
    name="Act",
    help="Actualizar paquetes, use 'todo' para actualizar todos los paquetes.",
)
def actualizar_arg(
    paquetes: list[str],
    verbose: Annotated[bool, Option(help="Mostrar información extra.")] = False,
    force: Annotated[
        bool, Option(help="Forzar la actualización en caso de error.")
    ] = False,
    ignorados: Annotated[
        bool, Option(help="Actualizar los paquetes ignorados.")
    ] = False,
    ignore_diff: Annotated[
        bool, Option(help="No mostrar el diff de cambios, solo el PKGBUILD.")
    ] = False,
    ignore_pkgbuild: Annotated[
        bool, Option(help="No muestra el PKGBUILD, solo el diff")
    ] = False,
...
desactualizados"
    ...
    solo_ignorados: Annotated[
        bool,
        Option(
            help="Actualizar SOLO los ignorados \
(No aplica a paquetes individuales)."
        ),
    ] = False,
):
...
instalado, pues falló el intentar eliminarlo con Pacman."
            ...
            red(
                "ERROR: Este paquete no se encuentra en la carpeta kpa, \
...
@cli.command(
    name="Limp",
    help="Limpiar paquetes huérfanos \
    ...
    for tipo in opciones:
        if tipo == "huerfanos":
            ...
            if value:
                try:
                    sb.run(
                        [datos["root"], "pacman", "-Rns", "--noconfirm"]
                        + huerfanos.split("\n"),
                        check=True,
                    )
                except sb.CalledProcessError as e:
...
def conf(
    to_set: str,
    list_conf: Annotated[
        bool, Option(help="Añadir a la lista en vez de sobreescribir")
    ] = False,
):
    ...
    try:
        ...
        with open(
            join(xdg_config_home, "kpa", "kpa.json"), "w", encoding="utf-8"
        ) as cf:
            json.dump(datos, cf, indent=4, ensure_ascii=False)
    except IndexError:
        red(
            "ERROR: El valor que intentaste configurar no tiene el estilo 'llave=valor'"
        )
